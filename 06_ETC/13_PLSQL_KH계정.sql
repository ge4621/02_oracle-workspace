/*
    ...
    SQL 문장내에서 변수의 정의, 조건처리(IF), 반복처리(LOOP, FOR, WHILE) 등을 지원하여 SQL의 단점 보안
    다수의 SQL문을 한번에 실행 가능(BLOCK 구조) + 예외 처리도 가능
...
--*간단하게 화면에 HELLO ORACLE 출력! (자바에서 HELLO WORLD 출력했던 것 처럼...)
BEGIN
...
/*
    1. DECLARE 선언부
    변수 및 상수 선언하는 공간(선언과 동시에 초기과 가능)
...
-------------------------------------------------------------------------------
--1_2) 레퍼런스 타입 변수 선언 및 초기화(어떤 테이블의 어떤 컬러의 데이터 타입을 참조해서 그 타입으로 지정
...
/*
    ...
    사용자가 입력한 사번의 사번, 사원명, 직급코드, 급여, 부서명, 조회 한 후 각 변수에 담아 출력
*/
...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('EID : ' || EID);
    DBMS_OUTPUT.PUT_LINE('ENAME : ' || ENAME);
    DBMS_OUTPUT.PUT_LINE('JOCODE : ' || JCODE);
    DBMS_OUTPUT.PUT_LINE('SAL : ' || SAL);
...
--단, 보너스를 받지 않는 사원은 보너스율 출력전 '보너스를 지급받지 않는 사원 입니다.' 출력
...
/
---------------------------------------------------------------------------------
--3) IF 조건식1 THEN 실행내용1 ELSIF 조건식2 THEN 실행내용2 ELSE 실행내용 END IF(IF-ELSE IF -ELSE문)
--점수를 입력받아 SCORE변수에 저장한 후
--90점 이상은 'A', 80점 이상은 'B', 70점 이상은 'C',60점 이상은 'D', 60점 미만은 'F'로 처리한후
-- GRADE변수로 저장
--당신의 점수는 XX점이고 , 학점은 X 학점 입니다.
...
DECLARE
    SAL EMPLOYEE.SALARY%TYPE;
    DNAME EMPLOYEE.EMP_NAME%TYPE;
    DBONUS EMPLOYEE.BONUS%TYPE;
BEGIN
    SELECT NAME, SALARY*12, NVL(BONUS,0)
    INTO DNAME , SAL,DBONUS
    FROM EMPLOYEE
    WHERE EMP_ID = &사번;
    
    
    IF BONUS = 0
        THEN DBMS_OUTPUT.PUT_LINE('연봉은 : ' || SALARY *12);
    ELSE 
        DBMS_OUTPUT.PUT_LINE('연봉은 : ' || (SALARY +SALARY * BONUS) *12);
    END IF;
END;
/
