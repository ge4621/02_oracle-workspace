/*
    ...
    (자주 쓰는 긴 SELECT문을 저장해주면 그 긴 SELECT문을 매번 다시 기술할 필요가 없다.)
...
/*
    *뷰 컬럼에 별칭 부여
     서브쿼리의 SELECT 절 함수식이나 산술연산식이 기술되어 있을 경우 반드시 별치 지정 해야 됨!!
*/
--전 사원의 사번, 이름, 직급명, 성별(남/여), 근무년수를 조회 할 수 있는 SELECT문을 뷰(VW_EMP_JOB)로 정의
...
--뷰를 통해 조작하더라도 실제 데이터가 담겨 있는 베이스테이블에 반영됨
 ...
 * 단 , DML명령어로 조작이 불가능한 경우가 더 많음!
    ...
    2)뷰에 정의되어있지 않은 컬럼 중에 베이스테이블 상에 NOT NULL 제약 조건이 지정되어 있는 경우 => 어떤건 되고 어떤건 안됨
...
DELETE FROM VW_JOB
WHERE JOB_NAME = '사원';
--ORA-02292: integrity constraint (KH.SYS_C007184) violated - child record found
...
--6)JOIN을 이용해서 여러 테이블을 연결시켜놓은 경우
CREATE OR REPLACE VIEW VW_JOINENP
AS SELECT EMP_ID, EMP_NAME, DEPT_TITLE
    FROM EMPLOYEE
    JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID);
    
SELECT * FROM VW_JOINENP;

--INSERT(에러)
INSERT INTO VW_JOINENP VALUES(300,'장원영','총무부');
...
SET DEPT_TITLE = '선동이'
WHERE EMP_ID = 200; --에러
--EMPLOYEE 에는 DEPT_TITLE 컬럼이 없다.

--DELETE
DELETE FROM VW_JOINENP
WHERE EMP_ID = 200;
...
-- FORCE : 서브쿼리에 기술된 테이블에 존재하지 않아도 뷰가 생성되게 하는 
CREATE OR REPLACE FORCE VIEW VW_EMP
AS SELECT TCODE, TNAME,TCONTENT
FROM TT;
--경고: 컴파일 오류와 함께 뷰가 생성되었습니다.

SELECT * FROM VW_EMP; --조회는 안된다.
