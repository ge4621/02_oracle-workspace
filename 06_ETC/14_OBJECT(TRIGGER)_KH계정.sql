/*
    ...
    내가 지정한 테이블에 INSERT, UPDATE, DELETE 등 DML문에 의해 변경사항이 생길때
    (테이블에 이벤트가 발생했을 때)
    자동으로 매번 실행할 내용을 미리 정의해둘 수 있는 객체
    
    EX)
    회원탈퇴시 기존의 회원테이블에 데이터를 DELETE 후 곧바로 탈퇴한 회원들만 따로 보관하는 테이블에 자동으로 INSERT처리
...
--2. 상품 입출고 상세이력 테이블(TB_PRODEATIL)
...
--TB_PRODEATIL 테이블에 INSERT 이벤트 발생시
...
/*
    ...
    --상품이 출고된 경우 => 해당 상품 찾아서 재소수량 감소 UPDATE
    UPDATE TB_PRODUCT
