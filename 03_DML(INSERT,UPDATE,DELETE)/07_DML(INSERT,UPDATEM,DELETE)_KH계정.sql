...
/*
    2) INSERT INTO 테이블명 (컬럼명, 컬럼명,컬럼명) VALUES(값1, 값2, 값3)
        ...
        단, DEFAULT값이 있는 경우 NULL이 아닌 DEFAULT값이 들어간다.
*/
...
/*
    ...
    SET 컬럼명 = 바꿀값,
        컬럼명 = 바꿀값,
        ...         --> 여러개의 컬럼값 동시변경 가능(,로 나열해야한다. AND아님!!!)
...
FROM EMP_SALARY
WHERE EMP_NAME = '방명수';--1669800, NULL

SELECT EMP_NAME, SALARY, BONUS
FROM EMP_SALARY
WHERE EMP_NAME = '유재식'; --4114000	0.2
...
SET JOB_CODE = 'J9'
WHERE EMP_NAME = '노옹철';
--ORA-02291: integrity constraint (KH.SYS_C007184) violated - parent key not found
--FROEIGN KEY 제약조건 위배!!
...
WHERE DEPT_ID = 'D1';
--ORA-02292: integrity constraint (KH.SYS_C007183) violated - child record found
--외래키 제약조건
--D1의 값을 가져다 쓰는 자식데이터가 있기때문에 삭제가 안됨!!
