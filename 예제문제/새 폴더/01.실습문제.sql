...
SELECT EMP_NAME, SALARY*12, (SALARY+SALARY*BONUS)*12 AS "총수령액", ((SALARY+SALARY*BONUS)*12 - (SALARY*12 *0.03)) AS "실수령액"
FROM EMPLOYEE;
...
SELECT EMP_ID,EMP_NAME,SUBSTR(EMP_NO,1,6) AS "주민번호 앞자리", SUBSTR(EMP_NO,8,14) AS "주민번호 뒷자리",
                        SUBSTR(EMP_NO,1,6) + SUBSTR(EMP_NO,8,14) AS "주민번호 앞 뒷자리 합"
FROM EMPLOYEE
WHERE EMP_ID = '201';
