/*
    ...
    *변경할 내용
    1) 컬럼 추가 / 수정 / 삭제
    2) 제약 조건 추가 / 삭제 --> 수정은 불가(수정하고자 한다면 삭제한 후 새로이 추가)
    3) 컬럼명 / 제약 조건명 / 테이블 명 수정
*/

--1) 컬럼 추가 / 수정 / 삭제
--1_1 컬럼 추가(ADD) : ADD 컬럼명 자료형 [DEFAULT 기본 값] 제약 조건
...
--이건 오류남! => 이미 데이터가 숫자가 아닌것도 들어 있음
--존재하는 데이터가 없어야만 이렇게 바꿀 수 있음

ALTER TABLE DEPT_COPY MODIFY DEPT_TITLE NARCHAR2(10);
--이것도 오류 발생
--이미 담겨있는 데이터가 10바이트 보다 크다
...
/*
    ...
    제약 조건명을 지정하고자 한다면 [CONSTRAINT 제약조건명] 제약조건
*/

--DEPT_ID에 PRIMARY KEY 제약 조건 추가 ADD
--DEPT_TITLE UNIQUE 제약 조건 추가 ADD
...
--2_2)제약 조건 삭제 : DROP CONSTRAINT 제약조건명
-- NOT NULL 삭제말과 MODIFY 컬럼명 NULL로 간다.
...
--------------------------------------------------------------------------------
--3) 컬럼명 / 제약 조건명 / 테이블명 변경(RENAME)
--3_1)컬럼명 변경 : RENAME COLUMN 기존컬럼명 TO 바꿀컬럼명

--DEPT_TITLE => DEPT_NAME
ALTER TABLE DEPT_COPY RENAME COLUMN DEPT_TITLE TO DEPT_NAME;

--3_2) 제약 조건명 변경 : RENAME CONSTRAINT 기존제약조건명 TO 바꿀제약조건명
--SYS_C007195 LOCATION_ID NN 자기꺼 가져오기
--SYS_C007195 => DCOPY_LID_NN
ALTER TABLE DEPT_COPY RENAME CONSTRAINT SYS_C007195 TO DCOPY_LID_NN;
...
--만약에 삭제하고자 한다면
--방법 1. 자식테이블 먼저 삭제 한 후 부모테이블 삭제하는 방법
