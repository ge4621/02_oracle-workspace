...
/*
    ...
    *자료형
    -문자 (CHAR(바이트 크기) | VARCHAR2(바이트크기)) => 반드시 크기 지정 해야한다.
    > CHAR : 최대 2000바이트까지 지정 가능. 지정한 범위 안에서만 써야한다. / 고정길이(지정한 크기보다 더 적은 값이 들어와도 공백으로 채워진다.)
            고정된 글자수의 데이터만이 담길 경우 사용 => 일반적으로 '한'글자.(Y N / M F)
            
    >VARCHAR2 : 최대 4000바이트 까지 지정 가능, 가변 길이(담긴 값에 따라 공간의 크기가 맞춰진다.)
                몇글자이 데이터가 들어올지 모르는 경우 + 긴글
...
--만약 컬럼명에 오타가 발생했다면?
--다시만들면 될까?? ㄴㄴ 삭제하고 다시하든 해야한다.....
--DROP TABLE 테이블명;

--DROP TABLE MEMBER;

--데이터 딕셔너리 : 다양한 객체들의 정보를 저장하고 있는 시스템 테이블들
--[참고] USER_TABLES : 현재 이 계정이 가지고 있는 테이블 구조 볼 수있다.

SELECT * FROM USER_TABLES;

--[참고] USER_TAB_CILUMNS : 이 사용자가 가지고 있는 테이블상의 모든 컬럼을 볼 수 있다.
SELECT * FROM USER_TAB_COLUMNS;

---------------------------------------------------------------------------------
/*
    2. 컬럼에 주석 달기(컬럼에 대한 설명 같은 거)
    [표현법]
    COMMENT ON COLUMN 테이블명.컬럼명 IS '주석내용';
    >>잘못 작성해서 실행했을 경우 수정후 다시 실행하면 됨
*/
...
/*
    *NOT NULL 제약조건
    ...
    제약조건을 부여하는 방식으로 크게 2가지가 있음(컬럼레벨방식/ 테이블레벨방식)
    *NOT NULL제약조건은 오로지 컴럼레벨방식 밖에 안됨*****

*/

--컬럼레벨 방식 : 컬럼명 자료형 제약 조건
...
/*
    *UNIQUE 제약조건
    ...
    삽입 / 수정시 기존에 있는 데이터 값 중 중복이 있을 경우 오류 발생!!
*/
...
CREATE TABLE MEM_UNIQUE( 
    ...
    MEM_PWD VARCHAR2(20) CONSTRAINT MEMWD_NN NOT NULL,
...
INSERT INTO MEM_UNIQUE VALUES(2,'user03','pass02','이승우','ㄴ',null,null);
...
--ORA-02290: check constraint (DDL.SYS_C007125) violated
...
/*
    *PRIMARY KEY(기본키)제약조건 => PK
    ...
    EX) 회원번호,학번, 사원번호, 부서코드, 직급코드,주문번호, 예약번호, 운소장번호
    
    PRIMARY KEY제약조건을 부여하면 그 컬럼에 자동으로 NOT NULL + UNIQUE 제약조건 가진다.
...
CREATE TABLE MEM_PRI( --컬럼레벨방식
    ...
    PHONE VARCHAR2(13),
    EMAIL VARCHAR2(50)
    --CONSTRATINT MEMNO_PK PRIMARY KEY(MEM_NO)
);
SELECT * FROM MEM_PRI;
...
CREATE TABLE MEM_PRI2( --컬럼레벨방식
    ...
    PHONE VARCHAR2(13),
    EMAIL VARCHAR2(50),
    PRIMARY KEY(MEM_NO,MEM_ID) --복합키=> 묶어서 PRIMARY KEY 제약 조건 부여
);
...
--PRIMARY KEY로 묶어있는 각 컬럼에는 절대 NULL을 허용하지 않음!!
...
INSERT INTO MEM
VALUES(3,'user03','pass03','이승우','남',null,null,40);
--ORA-02291: integrity constraint (DDL.SYS_C007152) violated - parent key not found
...
INSERT INTO MEM
VALUES(4,'user04','pass04','조규성',null,null,null,10);

--MEM_GRADE(부모테이블) MEM(자식테이블)
--이때 부모테이블(MEM_GRADE)에서 데이터 값을 삭제할 경우 어떤 문제가 발생할까??
--데이터 삭제 : DELETE FROM 테이블명 WHERE 조건;

--> MEM_GRADE테이블에서 10번 삭제
DELETE FROM MEM_GRADE
WHERE GRADE_CODE = 10;
--ORA-02292: integrity constraint (DDL.SYS_C007152) violated - child record found
-->자식테이블 (MEM)에 10이라는 값을 사용하고 있기 때문에 삭제가 안됨!!

DELETE FROM MEM_GRADE
WHERE GRADE_CODE = 30;
-->자식테이블(MEM)에 30이라는 값을 사용하고 있지 않기 때문에 삭제가 잘된다.!!!
...
/*
    ...
    *삭제 옵션 : 부모테이블의 데이터 삭제시 그 데이터를 사용하고 있는 자식테이블의 값을 
                어떻게 처리할껀지
    - ON DELETE RESTRICTED(기본값) : 삭제제한옵션으로, 자식데이터로 쓰이는 부모데이터는 삭제가 아예 안되게끔
    - ON DELETE SET NULL : 부모데이터를 삭제시 해당 데이터를 쓰고 있는 자식테이블의 값을 NULL로 변경
...
INSERT INTO MEMBER(MEM_NO,MEM_NAME) VALUES (4,'조규성');
SELECT * FROM MEMBER;
--선택되지 않은 컬럼 기본적으로 NULL이 들어간다.
--단, 해당 컬럼에 DEFAULT값이 있을 경우 NULL이 아닌 DEFAULT값이 들어간다.!!!
...
/*
    ...
    AS 서브쿼리;
*/
--EMPLOYEE테이블 복제한 새로운 테이블 생성
CREATE TABLE EMPLOYEE_COPY
AS SELECT * 
    FROM EMPLOYEE;

SELECT * FROM EMPLOYEE_COPY;
-->컬럼, 데이터 값, 제약조건 같은 경우 NOT NULL만 복사됨...

CREATE TABLE EMPLOYEE_COPY2
AS SELECT EMP_ID, EMP_NAME, SALARY, BONUS
    FROM EMPLOYEE --테이블 구조만 가져오고 싶다.
    WHERE 1 = 0; --무조건 FALSE인 조건 : 구조만을 복사하고자 할때 쓰이는 구문(데이터 값은 필요 없을 때)
...
--alias : 별칭
-->서브쿼리 SELECT 절에 산술식 또는 함수식 기술된 경우 반드시 별칭을 지정해야한다.
...
/*
    *테이블 다 생성된 후에 뒤늦게 제약조건 추가
    ...
    -FOREIGN KEY : ALTER TABLE 테이블명 ADD FROEIGN KEY(컬럼명) REFERENCES 참조할테이블명 [(참조할컬럼명)];
    -UNIQUE      : ALTER TABLE 테이블명 ADD UNIQUE(컬럼명);
    -CHECK       : ALTER TABLE 테이블명 ADD CHECK(컬럼에 대한 조건식);
    -NOT NULL    : ALTER TABLE 테이블명 MODIFY 컬럼명 NOT NULL; **약간특이함
...
ALTER TABLE EMPLOYEE ADD FOREIGN KEY(SAL_LEBEL) REFERENCES SAL_GRADE;

--DEPARTMENT 테이블에 LOCATION_ID에 외래키 제약조건 추가(LOCATEION참조)
ALTER TABLE DEPARTMENT ADD FOREIGN KEY(LOCATION_ID) REFERENCES LOCATION;

INSERT INTO DEPARTMENT VALUES('S1','테니스부','S1');
