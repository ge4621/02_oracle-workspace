...
SELECT DEPT_CODE,SUM(SALARY) --3번
FROM EMPLOYEE --1번
GROUP BY DEPT_CODE --2번
ORDER BY DEPT_CODE; --4번
...
SELECT JOB_CODE AS "직급명", COUNT(*)AS"직급별 총사원수",COUNT(BONUS) AS "보너스를 받는 사원수",SUM(SALARY)AS"급여 합",FLOOR(AVG(SALARY))AS "평균급여",MIN(SALARY)AS"최저급여",MAX(SALARY)AS"최대급여"
FROM EMPLOYEE
GROUP BY JOB_CODE
ORDER BY JOB_CODE;
...
GROUP BY DEPT_CODE; -- 오류 발생(그룹함수 가지고 조건 제시시 WHERE절에서 안된다.)

SELECT DEPT_CODE,AVG(SALARY) --4번
FROM EMPLOYEE --1번
GROUP BY DEPT_CODE --2번
HAVING AVG(SALARY) >= 3000000; --3번
...
FROM EMPLOYEE
WHERE DEPT_CODE = 'D5'; --6개행(박나라, 하이유, 김해슬. 심봉선, 윤은해, 대북혼)
...
FROM EMPLOYEE
WHERE SALARY>30000000;  --오류가 난다.
--각 쿼리문의 SELECT절에 작성되어 있는 컬럼개수가 동일해야한다.
...
--SELECT절에 작성되어 있는 컬럼개수가 동일해도 타입이 다르면 안된다.
--(컬럼개수 뿐만 아니라 각 컬럼 자리마다 동일한 타입으로 기술해야한다.)
...
WHERE SALARY>30000000
ORDER BY EMP_NAME;
--ORDER BY 절을 붙이고자 한다면 마지막에 기술해야한다.
--------------------------------------------------------------------------------
